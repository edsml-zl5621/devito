...
class PETScBasicHaloExchangeBuilder(HaloExchangeBuilder):
    ...
    def _make_all(self):
        ...
        # sendrecv = self._make_sendrecv(f, hse, key, msg=msg)
        gather = self._make_copy()
        # wait = self._make_wait(f, hse, key, msg=msg)
        # scatter = self._make_copy(f, hse, key, swap=True)
        # haloupdate = self._make_haloupdate(f, hse, key, sendrecv, msg=msg)
        # halowait = self._make_halowait(f, hse, key, wait, msg=msg)

        self._efuncs.append(gather)
        # if halowait is not None:
        #     self._efuncs.append(halowait)
        # if wait is not None:
        #     self._efuncs.append(wait)
    ...
    def _make_copy(self):
        ...
        grid = Grid(shape=(2, 2))
        func1 = Function(name='func1', grid=grid, space_order=2)
        func2 = Function(name= 'func2', grid=grid, space_order=2)
        tmp_eqn = [Eq(func1, func2.laplace)]
        # Compile `eqns` into an IET via recursive compilation
        irs, _ = self.rcompile(tmp_eqn, options={'mpi': False})

        parameters = [func1]

        return CopyBuffer('hellopetsc', irs.uiet, parameters)
...
mpi_registry = {
    ...
    'petsc': PETScBasicHaloExchangeBuilder
}
