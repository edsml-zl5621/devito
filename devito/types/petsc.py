...
from devito.tools import Reconstructable, as_tuple
...
class PETScFunction(LocalFunction):
    """
    PETScFunctions are generated by the compiler only but are
    built from user defined Devito Function objects.
...
def PETScSolve(eq, target, bcs=None, solver_parameters=None, **kwargs):
    ...
    y_matvec, x_matvec, b_tmp = [
        PETScFunction(name=f'{prefix}_{target.name}',
                      dtype=target.dtype, dimensions=target.dimensions,
                      shape=target.shape, liveness='eager', halo=target.halo)
