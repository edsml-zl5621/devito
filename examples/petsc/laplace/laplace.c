...
int Kernel(struct dataobj *restrict pn_vec, struct dataobj *restrict rhs_vec, const int x_M, const int x_m, const int y_M, const int y_m, struct MatContext * ctx, struct profiler * timers)
{
  ...
  PetscScalar** sol_tmp_pn;

  double (*restrict pn)[pn_vec->size[1]] __attribute__ ((aligned (64))) = (double (*)[pn_vec->size[1]]) pn_vec->data;
  ...
  PetscCall(DMSetFromOptions(da_pn));
  ...
  Vec tmp_local;
  PetscCall(DMCreateLocalVector(da_pn, &(tmp_local)));
  PetscCall(VecReplaceArray(tmp_local, pn_vec->data));
  PetscCall(VecView(tmp_local, PETSC_VIEWER_STDOUT_WORLD));
  ...
  PetscCall(KSPSolve(ksp_pn,b_pn,x_pn));
  PetscCall(DMDAVecGetArray(da_pn,x_pn,&sol_tmp_pn));
  for (int x = x_m; x <= x_M; x += 1)
  {
    for (int y = y_m; y <= y_M; y += 1)
    {
      pn[x + 2][y + 2] = sol_tmp_pn[x][y];
    }
  }
  PetscCall(DMDAVecRestoreArray(da_pn,x_pn,&sol_tmp_pn));
  ...
  PetscCall(MatDestroy(&(A_matfree_pn)));
  PetscCall(KSPDestroy(&(ksp_pn)));
