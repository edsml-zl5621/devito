...
PetscErrorCode FormFunction(SNES snes, Vec xvec_pn, Vec yvec_pn, void *null)
{
  ...
  for (int x = ctx->x_m; x <= ctx->x_M; x += 1)
  {
    for (int y = -ctx->i1y_rtkn + ctx->y_M + 1; y <= ctx->y_M; y += 1)
    {
      y_matvec_pn[x + 2][y + 2] = xvec_tmp_pn[x + 1][y + 2]/pow(ctx->h_x, 2) - 2.0*xvec_tmp_pn[x + 2][y + 2]/pow(ctx->h_x, 2) + xvec_tmp_pn[x + 3][y + 2]/pow(ctx->h_x, 2) + xvec_tmp_pn[x + 2][y + 1]/pow(ctx->h_y, 2) - 2.0*xvec_tmp_pn[x + 2][y + 2]/pow(ctx->h_y, 2) + xvec_tmp_pn[x + 2][2 + (int)(abs(y - 1))]/pow(ctx->h_y, 2);
    }
    for (int y = ctx->y_m; y <= ctx->i2y_ltkn + ctx->y_m - 1; y += 1)
    {
      y_matvec_pn[x + 2][y + 2] = xvec_tmp_pn[x + 1][y + 2]/pow(ctx->h_x, 2) - 2.0*xvec_tmp_pn[x + 2][y + 2]/pow(ctx->h_x, 2) + xvec_tmp_pn[x + 3][y + 2]/pow(ctx->h_x, 2) + xvec_tmp_pn[x + 2][2 + (int)(abs(y - 1))]/pow(ctx->h_y, 2) - 2.0*xvec_tmp_pn[x + 2][y + 2]/pow(ctx->h_y, 2) + xvec_tmp_pn[x + 2][y + 3]/pow(ctx->h_y, 2);
    }
  }

  for (int x = ctx->x_m; x <= ctx->i3x_ltkn + ctx->x_m - 1; x += 1)
  {
    for (int y = ctx->y_m; y <= ctx->y_M; y += 1)
    {
      y_matvec_pn[x + 2][y + 2] = 0.;
    }
  }

  for (int x = -ctx->i4x_rtkn + ctx->x_M + 1; x <= ctx->x_M; x += 1)
  {
    for (int y = ctx->y_m; y <= ctx->y_M; y += 1)
    {
      y_matvec_pn[x + 2][y + 2] = 0.;
...
PetscErrorCode MyMatShellMult_pn(Mat A_matfree_pn, Vec xvec_pn, Vec yvec_pn)
{
  ...
  for (int x = ctx->x_m; x <= ctx->i3x_ltkn + ctx->x_m - 1; x += 1)
  {
    for (int y = ctx->y_m; y <= ctx->y_M; y += 1)
    {
      y_matvec_pn[x + 2][y + 2] = - 2.0*xvec_tmp_pn[x + 2][y + 2]/pow(ctx->h_x, 2) - 2.0*xvec_tmp_pn[x + 2][y + 2]/pow(ctx->h_y, 2);
    }
  }
