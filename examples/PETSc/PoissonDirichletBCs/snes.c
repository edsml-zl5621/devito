...
int Kernel(struct dataobj *restrict pn_vec, struct dataobj *restrict rhs_vec, const int x_M, const int x_m, const int y_M, const int y_m, struct MatContext * ctx, struct profiler * timers)
{
  ...
  PetscCall(DMDACreate2d(PETSC_COMM_SELF,DM_BOUNDARY_GHOSTED,DM_BOUNDARY_GHOSTED,DMDA_STENCIL_BOX,11,11,PETSC_DECIDE,PETSC_DECIDE,1,2,NULL,NULL,&(da_pn)));
  ...
  PetscScalar (* b_tmp_pn)[15] = (PetscScalar (*)[15])b_one_dim;
...
PetscErrorCode preconditioner_callback_pn(Mat A_matfree_pn, Vec yvec_pn)
{
  ...
  PetscScalar (* y_pre_pn)[15] = (PetscScalar (*)[15])y_one_dim;
...
PetscErrorCode FormFunction(SNES snes, Vec xvec_pn, Vec yvec_pn, void *null)
{
  ...
  PetscScalar (* xvec_tmp_pn)[15] = (PetscScalar (*)[15])x_one_dim;
  PetscScalar (* y_matvec_pn)[15] = (PetscScalar (*)[15])y_one_dim;
